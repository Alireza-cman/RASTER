...
# @njit("float32[:,:](float32[:,:],Tuple((int32[:],int32[:],int64[:],float32[:])), unicode_type)", fastmath = True, parallel = True, cache = True)
def transform_taster_refined(X, parameters,function_name='ter'):
        ...
        for dilation_index in range(num_dilations):
            ...
            for kernel_index in range(num_kernels):
                ...
                for feature_count in range(num_features_this_dilation):
                    ...
                    start_feature = feature_index_start + feature_count 
                    c_size = random_size[start_feature]
                    end_feature = start_feature + c_size
                    beh = biases[start_feature:end_feature]
                    segments = len(beh)
                    M = len(C)//segments                
                    for portion in range(segments):
                        ...
                        if function_name == 'ter':
                            features[example_index, feature_index_start + feature_count+portion] = _PPV(C, beh_tmp).mean()
                        elif function_name == 'relu':
                            features[example_index, feature_index_start + feature_count+portion] = _ReLU(C, beh_tmp).mean()
                        else:
                            features[example_index, feature_index_start + feature_count+portion] = _PPV(C, beh_tmp).mean()
...
##
@njit("float32[:,:](float32[:,:],Tuple((int32[:],int32[:],int64[:],float32[:])), unicode_type)", fastmath = True,parallel = True , cache = True)
def transform_taster(X, parameters,function_name='ter'):
   

    num_examples, input_length = X.shape

    dilations, num_features_per_dilation,random_sizes, biases = parameters
    ...
    # num_features = num_kernels * np.sum(num_features_per_dilation)
    num_features = np.sum(random_sizes)
    ...
    for example_index in prange(num_examples):
        ...
        for dilation_index in range(num_dilations):
            ...
            for kernel_index in range(num_kernels):
                ...
                # my_bias = np.array(biases[feature_index_start + feature_count])
                # curve = create_rter(chunk = my_bias,length=len(C))
                ...
                for feature_count in range(num_features_this_dilation):
                    ...
                    my_size = random_sizes[feature_index_start + feature_count]
                    my_portion = len(C)//my_size
                    my_bias = biases[feature_index_start + feature_count:feature_index_start + feature_count+my_size]

                    # curve = create_rter(chunk = my_bias,length=len(C))
                    for s in range(len(my_bias)):
                        
                        c_temp = C[s*my_portion:(s+1)*my_portion]
                        
                        ss = my_bias[s]
                        curve = create_rter(chunk = [ss],length=len(c_temp))
                        if function_name == 'ter':
                            # features[example_index, feature_index_start + reseter + s] = _PPV(c_temp, curve).mean()
                            features[example_index, my_counter] = _PPV(c_temp, curve).mean()
                        else:
                            # features[example_index, feature_index_start + reseter + s] = _PPV(c_temp, curve).mean()
                            features[example_index, my_counter] = _PPV(c_temp, curve).mean()
                        my_counter+= 1 
